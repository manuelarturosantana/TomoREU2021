...
%Intialization for Irons and Tuck Acceloration
x_curr = [x0;RParams';angleParams'];
BCDinfo.x = x0;

%This enters the AABCD optimization loop.
for k = 1:optIter
    [G, iterInfo] = fpBCD_var(BCDinfo);
    delta_x = G - x_curr;
    %Change the x value in the BCD info, realling that G is [x Rparams
    %angParams]
    BCDinfo.x = G(1:length(x0));
    %This is G(G(x_n))
    GoG = fpBCD_var(BCDinfo);
    delta_G = GoG - G;
    %The second difference quoient of x
    delta_x2 = delta_G - delta_x;
    %The better version
    x_curr = GoG - dot(delta_G, delta_x2)/dot(delta_x2,delta_x2) * delta_G;
